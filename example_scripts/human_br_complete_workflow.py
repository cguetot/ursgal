...
def main(folder):
    ...
    uc = ursgal.UController(
        params={"enzyme": "trypsin", "decoy_generation_mode": "reverse_protein"}
    )
    ...
    for engine, wen_params, instrument in search_engine_settings:
        ...
        for mzML_file in mass_spec_files:
            unified_search_results = uc.search(input_file=mzML_file, engine=engine)
        ...
        merged_unvalidated_csv = uc.execute_misc_engine(
            input_file=unified_results, engine="merge_csvs_1_0_0"
        )
    ...
    uc.combine_search_results(
        input_files=merged_validated_files_5_engines, engine="combine_FDR_0_1"
    )

    uc.params["prefix"] = "3-engines-summary"
    uc.combine_search_results(
        input_files=merged_validated_files_3_engines, engine="combine_FDR_0_1"
    )
