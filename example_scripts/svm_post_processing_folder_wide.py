...
def main(folder=None, profile=None, target_decoy_database=None):
    ...
    uc = ursgal.UController(profile=profile, params=input_params)
    ...
    for search_engine in engine_list:
        ...
        for mzML_file in mzML_files:
            unified_search_result_file = uc.search(
                input_file=mzML_file, engine=search_engine
            )
        ...
        svm_validated_csv = uc.validate(
            input_file=merged_unified_csv, engine="svm_1_0_0"
        )
        ...
        svm_results_with_fdr = uc.add_estimated_fdr(input_file=svm_validated_csv)
