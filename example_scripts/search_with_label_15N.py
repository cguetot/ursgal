...
def main():
    ...
    engine_list = ["omssa_2_1_9", "xtandem_piledriver", "msgfplus_v9979"]
    ...
    params = {
        ...
        "ftp_include_ext": ["JB_FASP_pH8_2-3_28122012.mzML"],
    ...
    mgf_file = uc.convert(input_file=mzML_file, engine="mzml2mgf_1_0_0")
    files_2_merge = {}
    label_list = ["14N", "15N"]
    for label in label_list:
        ...
        for engine in engine_list:
            search_result = uc.search_mgf(input_file=mgf_file, engine=engine)
            converted_result = uc.convert(input_file=search_result, guess_engine=True)
            ...
            validated_file = uc.validate(
                input_file=unified_search_results, engine="percolator_2_08"
            )
        ...
        uc.visualize(
            input_files=validated_and_filtered_files_list, engine="venndiagram"
        )
    ...
    uc.visualize(input_files=label_comparison_file_list, engine="venndiagram")
