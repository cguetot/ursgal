...
def search(validation_engine):
    ...
    def workflow(
        ...
        for search_engine in search_engines:
            ...
            unified_search_results = uc.search(
                input_file=mgf_spec_file, engine=search_engine, force=force
            )
            uc.params["prefix"] = ""
        ...
        validated_results_from_all_engines = uc.execute_misc_engine(
            input_file=sorted(validated_results), engine="merge_csvs_1_0_0", force=force
        )
        uc.params["csv_filter_rules"] = [
    ...
    results_all_files = uc.execute_misc_engine(
        input_file=sorted(result_files), engine="merge_csvs_1_0_0"
    )
