...
def main():
    ...
    params = {
        ...
        "modifications": ["*,opt,Prot-N-term,Acetyl", "M,opt,any,Oxidation"],
    ...
    for engine in engine_list:
        ...
        unified_result_file = uc.search(input_file=mzML_file, engine=engine)
        time_collector[engine] = time.time() - start_time
        validated_file = uc.validate(
            input_file=unified_result_file, engine="percolator_2_08"
        )

        filtered_file = uc.execute_misc_engine(
            input_file=validated_file, engine="filter_csv_1_0_0"
        )

        filtered_files_list.append(filtered_file)

    uc.visualize(input_files=filtered_files_list, engine="venndiagram")
