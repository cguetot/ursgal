...
[ unifycsv ] Converting {0} of engine {1} to unified CSV format...
        """.format(
            os.path.basename(input_file), search_engine
        )
    )
    ...
    n_term_replacement = {"Ammonia-loss": None, "Trimethyl": None, "Gly->Val": None}
    fixed_mods = {}
    opt_mods = {}
    mod_dict = {}
    cam = False
    ...
    open_mod_search_engines = ["pipi", "moda"]
    de_novo = False
...
[ WARNING ] Could not find scan ID {0} in scan_rt_lookup[ {1} ]
            ...
            #########################
            main_buffer_key = (
                "{Sequence} || {Charge} || {Modifications} || ".format(**line_dict)
                + params["label"]
            )
            if main_buffer_key not in ze_only_buffer.keys():
                ...
                if use15N:
                    if (
                        "myrimatch" in search_engine.lower()
                        or "msgfplus_v9979" in search_engine.lower()
                    ):
                        for p in range(1, len(line_dict["Sequence"]) + 1):
                            line_dict["Modifications"] = line_dict[
                                "Modifications"
                            ].replace("unknown modification:{0}".format(p), "", 1)
...
[ WARNING ] This PSM will be skipped.
                    ...
                    if peptide_fullfills_enzyme_specificity is False:
                        ...
                        conflicting_uparams[lookup_identifier].add("enzyme")
        ...
        for collected_line_dict in line_dict_collector:
            ...
            if pyqms_mz_calc:
                ...
                calc_mz = ursgal.ucore.calculate_mz(
                    mass, int(collected_line_dict["Charge"])
                )
                min_accuracy = (
                    (float(collected_line_dict["Exp m/z"]) - calc_mz) / calc_mz * 1e6
                )
                for isotope in range(1, 6):
                    iso_mz = ursgal.ucore.calculate_mz(
                        mass + isotope * 1.008664904, int(collected_line_dict["Charge"])
                    )
                    isotopologue_acc = (
                        (float(collected_line_dict["Exp m/z"]) - iso_mz) / iso_mz * 1e6
                    )
