...
def main(input_file=None, output_file=None, params=None):
    ...
    if params["translations"]["peptide_mapper_class_version"] == "UPeptideMapper_v2":
        ...
        fasta_lookup_name = upapa.build_lookup_from_file(
            params["translations"]["database"], force=False
        )
...
[ WARNING ] Please adjust upeptide_mapper accordingly""".format(
        ...
        print(
            """[ map_peps ] Mapping {0} peptides using mapper {1}""".format(
                num_peptides, params["translations"]["peptide_mapper_class_version"]
            )
        )
...
[ WARNING ] This PSM will be skipped.
                ...
                if len(protein_mapping_dict["Protein ID"]) >= 2000:
                    print(
                        "{0}: {1}".format(
                            line_dict["Sequence"], protein_mapping_dict["Protein ID"]
                        ),
...
[ WARNING ] Check Search and Database if neccesary""".format(
        ...
        for non_mappable_peptide in non_mappable_peps:
            ...
            for pos, aa in enumerate(non_mappable_peptide):
                variants.add(
                    "{0}{1}{2}".format(
                        non_mappable_peptide[:pos], "X", non_mappable_peptide[pos + 1 :]
                    )
                )
            # reset buffer...
            upapa.peptide_2_protein_mappings[fasta_lookup_name] = defaultdict(list)
            p2p_mappings = upapa.map_peptides(list(variants), *class_etxra_args)
...
[ WARNING ] {2} are part of the non-mappable peptides""".format(
                ...
                if len(peptide_has_X_in_sequence) < 100
                else sorted(list(peptide_has_X_in_sequence))[:99],
...
[ WARNING ] Check of Search parameters and database is strongly recommended""".format(
                    ...
                    if len(not_mappable_after_all) < 100
                    else list(not_mappable_after_all)[:99],
...
class UPeptideMapper_v2(dict):
    ...
    def build_lookup_from_file(self, path_to_fasta_file, force=True):
        ...
        with open(abs_path, "r") as io:
            self.build_lookup(
                fasta_name=internal_name, fasta_stream=io.readlines(), force=force
            )
    ...
    def build_lookup(self, fasta_name=None, fasta_stream=None, force=True):
        ...
        if force:
            for upapa_id, (id, seq) in enumerate(
                ...
                self._create_fcache(id=id, seq=seq, fasta_name=fasta_name)
                # if upapa_id > 10000:
                #     break
            print()
        # input('bp')
    ...
    def _format_hit_dict(self, seq, start, end, id):
        ...
        if end >= len(seq):
            ...
            post_aa = seq[end]
        hit = {"start": start, "end": end, "id": id, "pre": pre_aa, "post": post_aa}
        return hit
...
if __name__ == "__main__":
    ...
    params = {
        "translations": {
            ...
            "aa_exception_dict": {
                "J": {"original_aa": ["I", "L"]},
                "O": {"original_aa": ["K"], "unimod_name": "Methylpyrroline"},
            },
            "protein_delimiter": "<|>",
            "decoy_tag": "decoy_",
            "word_len": 6,
    ...
    main(input_file=sys.argv[1], output_file=sys.argv[2], params=params)
