...
class Meta_UNode(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        obligatory_methods = ["preflight", "postflight"]
...
[ WARNING ] automatcally based on the last engine: {},
            ...
            if self.lookups["peptide_pos_buffer"][buffer_key][0] is None:
                self.print_info(
                    "Peptide {0} cant be found in database {1}, peptide for regex: {2}".format(
                        peptide, self.params["database"], peptide_for_regex
                    ),
                    caller="Warning",
                )
    ...
    def run(self, json_path=None):
        ...
        tag = "{0}@{1}".format(self.engine, self.io["input"]["finfo"]["full"])
        ...
        map_mods_node_exceptions = ["unify_csv"]
        ...
        if self.io["output"]["finfo"]["is_compressed"]:
            ...
            self.print_info(
                "Compressing output {0} > {1}".format(
                    os.path.basename(raw_output), os.path.basename(gz_output_file)
                )
            )
