...
class pepnovo_3_1(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "version": "v3.1",
        "release_date": None,
        "engine_type": {"de_novo_search_engine": True},
        ...
        "engine": {
            ...
            "darwin": {"64bit": {"exe": "PepNovo_bin"}},
            "win32": {"64bit": {"exe": "PepNovo.exe"}},
        },
    ...
    def _execute(self):
        if len(self.params["command_list"]) != 0:
            proc = subprocess.Popen(self.params["command_list"], stdout=subprocess.PIPE)
        else:
        ...
        with open(filepath, "r") as pepnovo_outputfile:
            for line in pepnovo_outputfile.readlines():
                if line.startswith(">>"):
                    spectrum_count += (
                        1
                    )  # just counts the number of read spectra, so that the spectrum ids can be correctly accessed in id_list
                    continue
                if line.startswith("#Index"):
                    save_data = (
                        True
                    )  # specifies that the next lines should be saved in result_dict
                    result_line = []
                    continue
                if line != "\n" and save_data == True:
