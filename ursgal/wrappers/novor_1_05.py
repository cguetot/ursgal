...
class novor_1_05(ursgal.UNode):
    ...
    META_INFO = {
        ...
        "engine": {
            "linux": {"64bit": {"exe": "novor.sh"}},
            "darwin": {"64bit": {"exe": "novor.sh"}},
            "win32": {"64bit": {"exe": "novor.bat"}},
        },
    ...
    def preflight(self):
        ...
        for mod_type in [potential_mods, fixed_mods]:
            ...
            for mod in not_available_mods.keys():
                if (
                    "{0} ({1})".format(mod, "".join(sorted(not_available_modsp[mod])))
            ...
            [ WARNING ] Continue without modification {0} ({1})""".format(
                            mod, "".join(sorted(not_available_modsp[mod]))
                        )
                    )
                    continue
                else:
    ...
    def postflight(self):
        ...
        translated_headers += ["Spectrum Title", "Modifications", "Raw data location"]
